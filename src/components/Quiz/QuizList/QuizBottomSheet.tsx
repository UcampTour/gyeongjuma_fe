...
const QuizBottomSheet = ({ quiz, drawerOpen, drawerClose }: QuizBottomSheetProps) => {
  ...
  return (
    <Sheet
      isOpen={drawerOpen}
      onClose={drawerClose}
      snapPoints={[0, 0.8, 1]} // 높이를 더 높게 설정
      initialSnap={1}
      disableDrag={false}
    >
      <Sheet.Container
      ...
      >
        <Sheet.Header />
        <Sheet.Content style={{ overflowY: 'auto', paddingBottom: '20px' }}>
          <Box sx={{ maxWidth: '500px', mx: 'auto', width: '100%', px: 3, pt: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', bgcolor: '#F9F7F2', p: 1.5, borderRadius: '20px' }}>
              {[
            ...
            {/* 여기 Box 고정 높이를 유지하여 레이아웃 흔들림 방지 */}
